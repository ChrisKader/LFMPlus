...
function lib:ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
  ...
  --[[ local ddSearch = _G["L_DropDownListSearch"]
  if not ddSearch then
    ddSearch = CreateFrame("EditBox","L_DropDownListSearch",listFrame,"InputBoxTemplate")
    ddSearch:Hide()
    ddSearch:SetFrameStrata("DIALOG")
    ddSearch:SetSize(50,50)
    ddSearch:SetAutoFocus(false)
    ddSearch:SetText("")
    ddSearch:SetPoint("TOP",listFrame,"BOTTOM")
    ddSearch:SetScript("OnTextChanged",function(self,input)
      local currentText = self:GetText()
      local foundButtons = 1
      for i=2,L_DropDownList1.numButtons do
        local currentButton = _G["L_DropDownList1Button"..i]
        if #currentText == 0 then
          currentButton:Show()
          foundButtons = foundButtons + 1
        elseif (currentButton.value and string.lower(currentButton.value):find(string.lower(currentText))) then
          foundButtons = foundButtons + 1
          if not currentButton:IsShown() then
            currenButton:Shown()
          end
          currentButton:ClearAllPoints()
          local ypos = -(L_UIDROPDOWNMENU_BUTTON_HEIGHT * foundButtons) - L_UIDROPDOWNMENU_BORDER_HEIGHT
          currentButton:SetPoint("TOPLEFT",listFrame,"TOPLEFT",17,ypos)
          foundButtons = foundButtons + 1
        else
          currentButton:Hide()
        end
      end
      listFrame:SetHeight(L_UIDROPDOWNMENU_BUTTON_HEIGHT * foundButtons);
    end)
  end ]]
